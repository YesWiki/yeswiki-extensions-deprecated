...
function collaborative_doc(&$formtemplate, $tableau_template, $mode, $valeurs_fiche) {
    ...
    elseif ($mode == 'requete') {	
    	...
    	if (!isset($valeurs_fiche[$tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url']) || $valeurs_fiche[$tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url']=='') {
    		if ($valeurs_fiche[$tableau_template[1]] == 'etherpad') {
    			return array($tableau_template[1] => $valeurs_fiche[$tableau_template[1]], $tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url' => 'http://pad.coop-tic.eu/p/paepard'.genere_nom_wiki( $valeurs_fiche['bf_titre'] ));
    		}
    		else {
				...
				$parent->setId($GLOBALS['wiki']->config['folder_id']); // cet id correspond à un dossier partage de paepard@gmail.com (avec le $GLOBALS['wiki']->config['service_account_name'] en partage)
				...
				return array($tableau_template[1] => $valeurs_fiche[$tableau_template[1]], $tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url' => $createdFile['alternateLink']);
    		}
		}
		else return array($tableau_template[1] => $valeurs_fiche[$tableau_template[1]], $tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url' => $valeurs_fiche[$tableau_template[1].'_url']);
    } 
    elseif ($mode == 'html') {
        $html = '';
        if (isset($valeurs_fiche[$tableau_template[1]]) && $valeurs_fiche[$tableau_template[1]]!='') {
        ...
        if (isset($valeurs_fiche[$tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url']) && $valeurs_fiche[$tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url']!='') {
            $html .= '<div class="BAZ_rubrique" data-id="'.$tableau_template[1].'_url'.'">'."\n".
                    ...
                    '<span class="BAZ_texte"><a href="'.$valeurs_fiche[$tableau_template[1].$valeurs_fiche[$tableau_template[1]].'_url'].'" target="_blank">'."\n".
